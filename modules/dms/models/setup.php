            ...
            if (Login::checkPermission($login, 'can_upload_dms') && Login::notDemoMode($login)) {
                ...
                if (preg_match('/^[0-9,]+$/', $id)) {
                    if ($action === 'delete') {
                        ...
                        // ลบข้อมูล
                        $this->db()->delete($this->getTableName('dms'), ['id', $ids], 0);
                        $this->db()->delete($this->getTableName('dms_files'), ['dms_id', $ids], 0);
                        $this->db()->delete($this->getTableName('dms_download'), ['dms_id', $ids], 0);
                        $this->db()->delete($this->getTableName('dms_meta'), ['dms_id', $ids], 0);
                        // Log
