...
class Model extends \Kotchasan\Model
{
    ...
    public static function toDataTable($params, $login)
    {
        ...
        if (!empty($params['from'])) {
            $where[] = ['A.create_date', '>=', $params['from']];
        }
        if (!empty($params['to'])) {
            $where[] = ['A.create_date', '<=', $params['to']];
        }
        $select = ['A.id', 'A.create_date', 'A.document_no', 'A.topic'];
        $query = static::createQuery()
            ->from('dms A');
        $n = 1;
        foreach (Language::get('DMS_CATEGORIES', []) as $k => $label) {
            $query->join('dms_meta N'.$n, 'LEFT', [['N'.$n.'.dms_id', 'A.id'], ['N'.$n.'.type', $k]])
                ->join('category C'.$n, 'LEFT', [['C'.$n.'.category_id', 'N'.$n.'.value'], ['C'.$n.'.type', $k]]);
            if ($k == 'department') {
                $select[] = Sql::GROUP_CONCAT('C'.$n.'.topic', $k);
                if (!empty($login['department'])) {
                    $where[] = ['N'.$n.'.value', $login['department']];
                }
            } else {
                $select[] = 'C'.$n.'.topic '.$k;
            }
            if (!empty($params[$k])) {
                $where[] = ['N'.$n.'.value', $params[$k]];
            }
            $n++;
        }
        $select[] = 'A.url';
        ...
        return static::createQuery()
            ...
            ->join('dms_download W', 'LEFT', [['W.file_id', Sql::create('CASE WHEN A.`url`="" THEN F.`id` ELSE 0 END')], ['W.dms_id', 'A.id'], ['W.member_id', $login['id']]]);
    }
    ...
    public function action(Request $request)
    {
        ...
        if ($request->initSession() && $request->isReferer() && $login = Login::isMember()) {
            if (Login::checkPermission($login, 'can_download_dms')) {
                ...
                if (preg_match('/(detail|download|manage)_([0-9]+)/', $request->post('action')->toString(), $match)) {
                    if ($match[1] == 'detail') {
