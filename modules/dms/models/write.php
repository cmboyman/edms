    ...
    public static function get($id, $login)
    {
        if (empty($id)) {
            ...
            foreach (Language::get('DMS_CATEGORIES', []) as $k => $label) {
                $query->join('dms_meta N'.$n, 'LEFT', [['N'.$n.'.dms_id', 'E.id'], ['N'.$n.'.type', $k]]);
                if ($k == 'department') {
                ...
                try {
                    ...
                    if ($index) {
                        ...
                        if ($save['document_no'] == '') {
                            ...
                            $search = $db->first($table_dms, ['document_no', $save['document_no']]);
                            if ($search && ($index->id == 0 || $index->id != $search->id)) {
                                $ret['ret_document_no'] = Language::replace('This :name already exist', [':name' => 'Document No.']);
                            }
                        }
                        if ($save['topic'] == '') {
                            // ไม่ได้กรอก topic
                        ...
                        if (empty($ret)) {
                            ...
                            if ($want == 'file') {
                                ...
                                // ลบไฟล์ (Database)
                                $db->delete($table_files, ['dms_id', $save['id']], 0);
                                // ลบไฟล์ (ถ้ามี)
                        ...
                        if (empty($ret)) {
                            ...
                            // meta
                            $db->delete($table_meta, ['dms_id', $save['id']], 0);
                            ...
                            $ret['location'] = $request->getUri()->postBack('index.php', ['module' => 'dms-setup']);
                            // เคลียร์
                            $request->removeToken();
                        }
                    }
